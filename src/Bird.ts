...
export class Bird implements IGameEntity {
    ...
    render(ctx: CanvasRenderingContext2D): void {
        ...
        const birdImage = resourceManager.get<HTMLImageElement>(`/bird/${this.facingDirection}flap.png`)
        if(!birdImage) return

        ctx.beginPath()
        ctx.drawImage(birdImage,this.position.x, this.position.y, this.width, this.height)
    ...
    update(dt: number) {
        ...
        const action = inputManager.getAction()
        if (action === "MOVE_UP") {
